...
DJ Roomba is a Discord music bot written in [Python](https://www.python.org) 3.10+, using the [discord.py](https://github.com/Rapptz/discord.py) library. It is a fork of [MusicBot](https://github.com/Just-Some-Bots/MusicBot) with additional commands, a cleaner visual style, and auto-deleting responses to keep your channels tidy. It is driven primarily by native Discord **slash commands** (`/play`, `/queue`, `/nowplaying`, and more) with argument hints and interactive buttons, while the classic prefix commands remain available as an alternative. It plays requested songs from YouTube and other supported services (including Spotify), falls back to a configurable auto playlist when the queue is empty, and includes a permission system so owners can restrict commands to specific roles or users.
...
The recommended way to control DJ Roomba is with **slash commands** (`/play`, `/queue`, `/nowplaying`, …). They register automatically when the bot joins a server and provide argument hints, along with interactive buttons for paging through the queue and picking from search results.

Every command is also available as a **prefix command** for those who prefer it. DJ Roomba uses `!` as the default prefix (configurable with `setprefix`), and many prefix commands have aliases (see [`config/example_aliases.json`](./config/example_aliases.json) for the defaults).

> The tables below list each command by name. Most work either way and share the same name: use `/play` or `!play`, and both run the same underlying action. Some exceptions: the queue-management commands use distinct slash names (noted in the table), commands marked **†** are available as prefix commands only, and a few slash commands (`/queue`, `/search`, `/nowplaying`) add interactive buttons and menus that the prefix versions present as plain text.
...
| `/shufflequeue` (prefix: `shuffle`) | Shuffle the queue. |
| `/clearqueue` (prefix: `clear`) | Clear the queue. |
| `/removequeue <position>` (prefix: `remove`) | Remove a song from the queue by position. |
...
**†** Available as a prefix command only.
