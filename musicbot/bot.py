            ...
            if guild.id not in self.players:
                if not create:
                    raise exceptions.CommandError(
                        ...
                        'Use `%ssummon` or `%splay [song query]` to summon me to your voice channel.' % (
                        self.config.command_prefix, self.config.command_prefix), expire_in=20)
...
#######################################################################################################################
    ...
    def _gen_embed(self):
        ...
        e.set_footer(text=self.config.footer_text,
                     icon_url='https://i.imgur.com/D4thUUu.png')
        e.set_author(name=self.user.name, url='https://github.com/Jimgersnap/DJ-Roomba',
    ...
    async def cmd_botinfo(self, message):
        ...
        return Response(
            'DJ Roomba is a Python-based Discord music bot, a fork of Just-Some-Bots/MusicBot, with many additional features and a cleaner Discord presence. '
            'This instance of DJ Roomba is running version `1.6.0` and is running on top of MusicBot/review.'
            '\n\nDJ Roomba GitHub Repo: `https://github.com/Jimgersnap/DJ-Roomba`'
            '\nMusicBot GitHub Repo: `https://github.com/Just-Some-Bots/MusicBot`'
            '\nTeam-JSB Forked GitHub Repo: `https://github.com/Team-JSB/MusicBot`'
    ...
    async def _cmd_play(
                ...
                try:
                    if "track" in parts:
                        ...
                        return Response(self.str.get(
                            'cmd-play-spotify-album-queued', "Your requested Spotify album (**{0}**) containing **{1}** songs has been added to the queue. Shuffle the queue by typing `{2}shuffle`.").format(res['name'], len(res['tracks']['items']), self.config.command_prefix), delete_after=20)
        ...
        async with self.aiolocks[_func_() + ":" + str(author.id)]:
            ...
            if not info:
                raise exceptions.CommandError(
                    self.str.get(
                        'cmd-play-noinfo', "Sorry, that video cannot be played. Is it a stream? If so, please use the {0}stream command.").format(self.config.command_prefix),
                    expire_in=30
                )
            ...
            if (
                ...
                raise exceptions.PermissionsError(
                    self.str.get('cmd-play-badextractor', "Sorry, but you do not have permission to play media from this service."), expire_in=30
                )
            ...
            if info.get("url", "").startswith("ytsearch"):
                ...
                if info_process:
                    ...
                    raise exceptions.CommandError(
                        self.str.get('cmd-play-nodata', "I'm having issues extracting the info from the search string since youtubedl returned no data and might be broken. "
            ...
            if "entries" in info:
                ...
                reply_text = self.str.get(
                    'cmd-play-playlist-reply', "Your requested playlist containing **%s** songs has been added to the queue.\nPosition in queue: **%s**")
            ...
            # If it's an entry
                ...
                if (
                    ...
                    raise exceptions.PermissionsError(
                        self.str.get('cmd-play-song-limit', "That song exceeds the maximum duration you are allowed to queue: `({0} > {1})` seconds.").format(
                ...
                reply_text = self.str.get(
                    'cmd-play-song-reply', "Added **%s** to the queue.\nPosition in queue: **%s**")
                btext = entry.title
    ...
    async def _cmd_play_playlist_async(
        ...
        if not info:
            raise exceptions.CommandError(self.str.get(
                'cmd-play-playlist-invalid', "Sorry, but that playlist cannot be played."))
        ...
        if extractor_type == "youtube:tab":
            try:
                ...
                raise exceptions.CommandError(self.str.get(
                    'cmd-play-playlist-queueerror', 'Oops. There has been an error handling the requested playlist `{0}`. Queue canceled. Please try another playlist.').format(
    ...
    async def cmd_playnow(self, player, channel, author, permissions, leftover_args, song_url):
        ...
        if not info:
            raise exceptions.CommandError(self.str.get('cmd-playnow-unplayable-entry', "Sorry, but I'm unable to play that video. Please try something else."), expire_in=30)
    ...
    async def cmd_stream(self, _player, channel, author, permissions, song_url):
        ...
        if _player:
            ...
            vc = author.voice.channel if author.voice else None
            response = await self.cmd_summon(channel, channel.guild, author, vc)
            if self.config.embeds:
                content = self._gen_embed()
                content.title = 'summon'
                content.description = response.content
    ...
    async def cmd_search(
        ...
        try:
            ...
            raise exceptions.CommandError(self.str.get(
                'cmd-search-noquote', "Please quote your search query properly and try again."), expire_in=30)
        ...
        if self.config.searchlist:
            ...
            if self.config.embeds:
                ...
                result_string += "\n\nSelect the song you want by typing the corresponding number or type cancel to cancel the search."
            ...
            if choice.content == "0":
                ...
                if self.config.embeds:
                    return Response(
                        self.str.get(
                            "cmd-search-accept-list-embed", "[{0}]({1}) added to the queue."
                        ).format(
                    ...
                    return Response(
                        self.str.get(
                            "cmd-search-accept-list-noembed", "{0} added to the queue."
            ...
            for e in info["entries"]:
                ...
                if str(reaction.emoji) == "\u2705":  # check
                    ...
                    return Response(
                        self.str.get("cmd-search-accept", "Your song has been added to the queue."),
                        delete_after=30,
                    )
        ...
        return Response(
            self.str.get("cmd-search-decline", "If the results you were expecting were not found, try again with a search query containing more info."), delete_after=30
        )
    ...
    async def cmd_nowplaying(self, player, channel, guild, message):
        ...
        if player.current_entry:
            ...
            return Response(
                self.str.get(
                    'cmd-np-none', 'There are no songs in the queue. Queue something with `{0}play`.') .format(self.config.command_prefix),
                delete_after=30
            )
    ...
    async def cmd_summon(self, channel, guild, author, voice_channel):
        ...
        if not author.voice:
            raise exceptions.CommandError(self.str.get(
                'cmd-summon-novc', 'You are not connected to voice. Please join a voice channel to summon me.'), expire_in=20)
        ...
        return Response(self.str.get('cmd-summon-reply',
        "Connected to `{0.channel}` on `{1.guild}`. Please type `{2}help` to learn more about what I can do.").format(
        author.voice.channel, author.voice.channel.guild, self.config.command_prefix), delete_after=30)
    ...
    async def cmd_pause(self, player):
        ...
        if player.is_playing:
            ...
            raise exceptions.CommandError(self.str.get(
                'cmd-pause-none', 'Nothing is currently playing.'), expire_in=30)
    ...
    async def cmd_resume(self, player):
        ...
        if player.is_paused:
            ...
            raise exceptions.CommandError(self.str.get(
                'cmd-resume-none', 'I am not paused.'), expire_in=30)
    ...
    async def cmd_shuffle(self, channel, player):
        ...
        return Response(self.str.get('cmd-shuffle-reply', "The queue in `{0}` has been shuffled.").format(player.voice_client.channel.guild), delete_after=15)
    ...
    async def cmd_remove(
        ...
        if not player.playlist.entries:
            raise exceptions.CommandError(self.str.get(
                'cmd-remove-none', "There is nothing in the queue to remove."), expire_in=20)
                ...
                if permissions.remove or author == user:
                    try:
                        ...
                        if len(entry_indexes) > 1:
                            entry_text += 's'
                        return Response(self.str.get('cmd-remove-reply',
                        "The song **{0}** added by **{1}** has been removed from the queue.").format(
                        entry_text, user.name).strip())

                    except ValueError:
                        raise exceptions.CommandError(self.str.get(
                            'cmd-remove-missing', "Nothing was found in the queue from user **%s**.") % user.name, expire_in=20)
        ...
        try:
            ...
            raise exceptions.CommandError(self.str.get(
                'cmd-remove-invalid', "Invalid number. Use `{}queue` to find the queue position values.").format(
        ...
        if permissions.remove or author == player.playlist.get_entry_at_index(
            ...
            if entry.meta.get('channel', False) and entry.meta.get('author', False):
                return Response(self.str.get('cmd-remove-reply-author', "Removed entry **{0}** added by **{1}**.").format(
                entry.title, entry.meta['author'].name).strip(), delete_after=20)
            else:
                return Response(self.str.get('cmd-remove-reply-noauthor',
                "Removed entry **{0}**.").format(entry.title).strip(), delete_after=20)
        else:
            raise exceptions.PermissionsError(
                self.str.get('cmd-remove-noperms',
    ...
    async def cmd_skip(
        ...
        if player.is_stopped:
            raise exceptions.CommandError(self.str.get(
                'cmd-skip-none', "There is nothing to skip because I am not playing anything."), expire_in=20)
            ...
            if player.playlist.peek():
                if player.playlist.peek()._is_downloading:
                    ...
                    print("Something odd is happening.  "
                          "You might want to restart me if I don't respond soon.")
            else:
    ...
    async def cmd_volume(self, message, player, new_volume=None):
        ...
        if not new_volume:
            return Response(self.str.get('cmd-volume-current', ', the current volume is **%s%%**.') % int(player.volume * 100), reply=True, delete_after=20)
        ...
        if 0 < new_volume <= 100:
            ...
            return Response(self.str.get(
            'cmd-volume-reply', ', the volume has been changed from **%d%%** to **%d%%** on your request.') % (old_volume, new_volume),
    ...
    async def cmd_queue(self, channel, player):
        ...
        for i, item in enumerate(player.playlist, 1):
            if item.meta.get('channel', False) and item.meta.get('author', False):
                nextline = self.str.get('cmd-queue-entry-author', '{0} -- **{1}** added by **{2}**').format(i, item.title, item.meta['author'].name).strip()
            else:
        ...
        if not lines:
            lines.append(
                self.str.get('cmd-queue-none', 'There are no songs in the queue. Queue something with `{}play`.').format(self.config.command_prefix))
