...
class MusicBot(discord.Client):
    ...
    async def handle_player_inactivity(self, player: MusicPlayer) -> None:
        ...
        if channel.id in self.config.autojoin_channels:
            log.debug(
    ...
    @command_helper(desc=_Dd("Returns general information about DJ Roomba."))
    async def cmd_botinfo(self, message: discord.Message) -> CommandResponse:
        """
    ...
    @command_helper(
        usage=["{cmd}", "{cmd} [POSITION]"],
        desc=_Dd(
            "Promotes the last song in the queue to the front.\n"
            "If a position is specified, promotes the song at that position to play next.\n"
        ),
    )
    async def cmd_promote(
        ...
        """
        Promotes a song to the top of the queue to play next.
        """
        ...
        if not position:
            # promote the last entry to the front
        ...
        try:
            ...
            eta_str = format_song_duration(time_until)
        except Exception:  # pylint: disable=broad-except
            eta_str = "unknown"

        return Response(
            f"**{entry.title}** has been promoted to the top of the queue.\n\nEstimated time until playing: `{eta_str}`",
            delete_after=20,
        )
    ...
    async def cmd_stop(
        ...
        """
        Skips the current song and clears the queue entirely.
        """
        if not author.voice:
            raise exceptions.CommandError(
        ...
        return Response(
            f"The current song has been stopped and the queue has been cleared in `{player.voice_client.channel.name}`.\n"
            f"Start a new session by queuing songs with `{self.config.command_prefix}play`.",
            delete_after=30,
        )

    @command_helper(
        desc=_Dd(
            "Disconnects from the current voice channel and reconnects.\n"
            "Useful if the bot becomes unresponsive or loses audio.\n"
        ),
    )
    ...
    async def cmd_stream(
        ...
        return Response(
            _D(
                "Connected to the stream successfully.\n**%(track)s**\n\nEnd the stream by typing `%(prefix)sskip`.",
                ssd_,
            )
            % {"track": info.title, "prefix": self.config.command_prefix},
        )
    ...
    @command_helper(desc=_Dd("Alias for the np command. Displays information on the currently playing song."))
    async def cmd_nowplaying(
        ...
        """
        Alias of cmd_np with DJ-Roomba branding.
        """
        return await self.cmd_np(ssd_, player, channel, guild)

    @command_helper(desc=_Dd("Shows a progress bar for the currently playing song."))
    async def cmd_songprogress(
        self,
        ssd_: Optional[GuildSpecificData],
        player: MusicPlayer,
    ) -> CommandResponse:
    ...
    async def cmd_shuffle(
        ...
        return Response(_D("The queue has been shuffled.", ssd_))
    ...
    async def cmd_clear(
        ...
        return Response(
            _D(
                "The queue has been cleared. Start a new session with `%(prefix)splay`.",
                ssd_,
            )
            % {"prefix": self.config.command_prefix}
        )
    ...
    async def cmd_remove(
        ...
        if len(leftover_args) == 1:
            ...
            return Response(
                _D(
                    "Removed songs **%(from)s** through **%(to)s** from the queue.",
                    ssd_,
                )
                % {"from": indexes[0] + 1, "to": indexes[1] + 1},
            )
                ...
                if permissions.remove or author == user:
                    try:
                        ...
                        if len(entry_indexes) > 1:
                            entry_text += "s"
                        return Response(
                            _D("Removed **%(track)s** items added by **%(user)s** from the queue.", ssd_)
        ...
        if (
            ...
            if entry.channel and entry.author:
                return Response(
                    _D("Removed **%(track)s** added by **%(user)s** from the queue.", ssd_)
                    % {"track": _D(entry.title, ssd_), "user": entry.author.name},
                )

            return Response(
                _D("Removed **%(track)s** from the queue.", ssd_)
